...
try:
	from modal_app.main import train  # type: ignore
...
reader_app = modal.App("ai-builder-reader")
shared_volume = modal.Volume.from_name(VOLUME_NAME, create_if_missing=True)

@reader_app.function(image=modal.Image.debian_slim(), volumes={MODELS_DIR: shared_volume}, timeout=60)
		...
		try:
			...
			if {"step", "train_loss"}.issubset(df.columns):
				with col2:
					...
					plot_df = plot_df.rename(columns={"train_loss": "Train Loss"})
